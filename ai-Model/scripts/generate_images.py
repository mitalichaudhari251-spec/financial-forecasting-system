...
Usage: python scripts/generate_images.py --ticker AAPL --method candlestick
"""
import argparse
import sys
...
def main():
    ...
    parser.add_argument("--method", default="candlestick", choices=["candlestick", "gaf"])
    args = parser.parse_args()

    log.info(f"Image Generation: {args.ticker} | method={args.method}")
    ...
    generator = CandlestickGenerator() if args.method == "candlestick" else GAFGenerator()
    images = generator.batch_to_images(windows)

    out_dir = IMAGES_DIR / args.method
    metadata = save_images(images, labels, out_dir, args.ticker)
    log.info(f"Generated {len(metadata)} images → {out_dir / args.ticker}")
