...
import ProbabilityChart from '@/components/forecasting/ProbabilityChart';
...
export default function ConfidencePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      toast.success(`${sym} confidence: ${result.confidence.toFixed(1)}%`);
    } catch (err) {
  ...
  return (
    <DashboardLayout>
      <PageContainer title="Confidence Metrics" subtitle="Model confidence scores and probability distributions"
        actions={
        ...
        {!forecast && !isRunning && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-[#9CA3AF]">Select a symbol and click Analyze</p>
          </div>
        )}
        ...
        {forecast && !isRunning && (
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-4">
            <motion.div variants={fadeInUp} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <ConfidenceGauge confidence={forecast.confidence} size={180} />
              <ProbabilityChart probabilities={forecast.probabilities} />
            </motion.div>
            <motion.div variants={fadeInUp} className="grid grid-cols-3 gap-3">
              {[
                { label: 'Bullish %', value: `${(forecast.probabilities.bullish * 100).toFixed(1)}%`, color: 'text-green-600' },
                { label: 'Neutral %', value: `${(forecast.probabilities.neutral * 100).toFixed(1)}%`, color: 'text-gray-600' },
                { label: 'Bearish %', value: `${(forecast.probabilities.bearish * 100).toFixed(1)}%`, color: 'text-red-600' },
              ].map(s => (
